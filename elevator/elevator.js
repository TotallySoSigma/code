...
class Elevator {
    ...
    getNextDestinationFloor() {
            ...
            for (let i = 0; i < this.floors.length; i++) {
                if (this.floors[i].upButtonPressed || this.floors[i].downButtonPressed) {
                    if (closestDestination != -1) {
                        closestDestination = Math.min(
                            Math.abs(this.floors[i].floorNumber - this.currentFloor),
                            closestDestination
                        );
                    } else {
        ...
        for (let i = 1; i < this.queue.length; i++) {
            closestDestination = Math.min(
                this.queue[i].destinationFloor - this.currentFloor,
                closestDestination
            );
        }
        return closestDestination;
    ...
    loadPeople() {
        ...
        if (this.queue.length < this.capacity) {
            for (let i = 0; i < currFloorQueue.length; i++) {
                if (
                    (currFloorQueue[i].destinationFloor > currentfloor && this.getNextDirection() == UP) ||
    ...
    generatePeopleRandomly(numPeople) {
        ...
        for (let i = 0; i < numPeople; i++) {
            for (let j = 0; j < this.numFloors; j++) {
                this.floors[j].addPerson(
                    new Person("Moo", getRandomInt(0, this.numFloors), getRandomInt(0, this.numFloors))
                );
            }
        }
    }
